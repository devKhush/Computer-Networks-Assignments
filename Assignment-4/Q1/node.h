...
protected:
  ...
  virtual void recvMsg(RouteMsg *msg, int cost)
  {
    cout << "Base" << endl;
  }
...
public:
  ...
  void sendMsg()
  {
    ...
    for (int i = 0; i < interfaces.size(); ++i)
    {
      // Cost to the node through the interface
      int destinationCost = 0;

      // Now e find the cost to the destination node through the active connected interface
      // by looking at the routing table
      for (RoutingEntry neighbourEntry : mytbl.tbl)
      {
        // Update the cost to the destination node through the active connected interface
        if (neighbourEntry.dstip == interfaces[i].first.getConnectedIp())
        {
          destinationCost = neighbourEntry.cost;
        }
      }

      // Now we send the message to the neighbour node
      RouteMsg msg;
      msg.from = interfaces[i].first.getip();
      // printf("i=%d, msg-from-interface=%s\n",i, msg.from.c_str());
      msg.mytbl = &ntbl;
      msg.recvip = interfaces[i].first.getConnectedIp();
      interfaces[i].second->recvMsg(&msg, destinationCost);
    }
  }
};
...
{
public:
  void recvMsg(RouteMsg *msg, int destinationCost);
};
