...
void RoutingNode::recvMsg(RouteMsg *msg, int destinationCost)
{
  ...
  for (RoutingEntry entry : recvRoutingTable->tbl)
  {
    ...
    for (int i = 0; i < mytbl.tbl.size(); ++i)
    {
      ...
      // Check if message is received from the same node
      if (myEntry.dstip == entry.dstip)
      {
        entryExists = true;
        // Check if the cost is less than the existing cost
    ...
    // If entry does not exist, then add the new entry which is the direct link cost from node to neighbour
    // in the routing table of the node.
    if (!entryExists)
    {
      // add the new entry
